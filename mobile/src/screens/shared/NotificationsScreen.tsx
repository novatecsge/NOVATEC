...
export const NotificationsScreen = () => {
  ...
  return <Screen><Header title="Notificaciones" subtitle="Avisos del sistema en tiempo real." /><Button title="Marcar todas como leídas" variant="outline" onPress={async()=>{ await notificationsService.markAllAsRead(); await load(); }} /><ListState loading={loading} empty={!loading && toArray(items).length===0} />{toArray<NotificationItem>(items).map((n) => <Card key={String(n.id || n.title)}><Text style={{ fontWeight:'900', color: colors.text }}>{String(n.title || 'Notificación')}</Text><Text style={{ color: colors.muted, marginVertical: 6 }}>{String(n.message || '')}</Text><Button title="Marcar leída" variant="outline" onPress={() => n.id ? read(n.id) : undefined} /></Card>)}</Screen>;
};
