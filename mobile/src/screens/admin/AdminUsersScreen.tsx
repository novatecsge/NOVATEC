...
export const AdminUsersScreen = () => {
  ...
  useEffect(() => { load(); }, []);
  return <Screen><Header title="Usuarios" subtitle="Consulta administrativa del mismo módulo web." /><Button title="Actualizar" loading={loading} onPress={load}/><ListState loading={loading} empty={!loading && toArray(items).length===0}/>{toArray<User>(items).map((u) => <Card key={String(u.id || u.email)}><Text style={{ fontWeight:'900', color: colors.text }}>{String(u.fullName || u.full_name || u.email)}</Text><Text style={{ color: colors.muted }}>{String(u.email)} · {String(u.role || u.role_name)}</Text></Card>)}</Screen>;
};
