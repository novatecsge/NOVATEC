        ...
        {data.map((item, index) => {
          ...
          return (
            <g key={`${item[xKey]}-${index}`}>
              ...
              <text x={x + (bars.length * (barWidth + 4)) / 2 - 3} y={height - 25} textAnchor="middle" fontSize="11" fill="var(--muted, #94a3b8)" fontWeight="800">
                {item[xKey]}
              </text>
            </g>
          );
      ...
      <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', minWidth: 680 }} role="img">
        ...
        {points.map((point, index) => (
          <g key={`${point.item[xKey]}-${index}`} onMouseEnter={() => setHover(point.item)} onMouseLeave={() => setHover(null)}>
            <circle cx={point.x} cy={point.y} r="6" fill="var(--surface-elevated, #111827)" stroke={color} strokeWidth="3" />
            <text x={point.x} y={height - 25} textAnchor="middle" fontSize="11" fill="var(--muted, #94a3b8)" fontWeight="800">{point.item[xKey]}</text>
          </g>
        ))}
      </svg>
...
export function PieDonutChart({ entries = 0, exits = 0, title = 'Distribución entradas/salidas' }) {
  const totalEntries = Number(entries || 0);
  const totalExits = Number(exits || 0);
  const total = Math.max(1, totalEntries + totalExits);
  const entryPct = totalEntries / total;
  const angle = entryPct * Math.PI * 2;
  const largeArc = entryPct > 0.5 ? 1 : 0;
  const x = 110 + 86 * Math.sin(angle);
  const y = 110 - 86 * Math.cos(angle);
  const entryPath = `M 110 110 L 110 24 A 86 86 0 ${largeArc} 1 ${x} ${y} Z`;

  return (
    <div className="pie-layout">
      <svg viewBox="0 0 220 220" style={{ width: '100%', maxWidth: 240 }} role="img" aria-label={title}>
        <circle cx="110" cy="110" r="86" fill="#10b981" />
        <path d={entryPath} fill="#2563eb" />
        <circle cx="110" cy="110" r="52" fill="var(--surface-elevated, #111827)" />
        <text x="110" y="104" textAnchor="middle" fontSize="13" fill="var(--muted, #94a3b8)" fontWeight="800">Total</text>
        <text x="110" y="126" textAnchor="middle" fontSize="24" fill="var(--text, #e5e7eb)" fontWeight="900">{totalEntries + totalExits}</text>
      </svg>
      <div className="pie-legend">
        <span><i style={{ background: '#2563eb' }} />Entradas: {totalEntries}</span>
        <span><i style={{ background: '#10b981' }} />Salidas: {totalExits}</span>
        <span><i style={{ background: '#e2e8f0' }} />Balance operativo mensual</span>
      </div>
    </div>
  );
}
